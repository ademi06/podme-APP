package uk.ac.tees.w9312536.bukolafatunde.service;
...
import uk.ac.tees.w9312536.bukolafatunde.ui.nowplaying.NowPlayingActivity;
import uk.ac.tees.w9312536.bukolafatunde.utilities.Constants;
...
public class PodcastService extends MediaBrowserServiceCompat implements Player.EventListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent.getAction() != null && intent.getAction().equals(Constants.ACTION_RELEASE_OLD_PLAYER)) {
            if (mExoPlayer != null) {
                mExoPlayer.stop();
                releasePlayer();
            }
        }
        Bundle b = intent.getBundleExtra(Constants.EXTRA_ITEM);
        if (b != null) {
            mItem = b.getParcelable(Constants.EXTRA_ITEM);
        ...
        if (intent.hasExtra(Constants.EXTRA_RESULT_NAME)) {
            mPodcastName = intent.getStringExtra(Constants.EXTRA_RESULT_NAME);
        }
        // Get the podcast image
        if (intent.hasExtra(Constants.EXTRA_PODCAST_IMAGE)) {
            mPodcastImage = intent.getStringExtra(Constants.EXTRA_PODCAST_IMAGE);
        }
        // Initialize ExoPlayer
        initializePlayer();
...
//        mMediaSession.setMetadata(metadata);
        ...
        String itemImageUrl = PodCastUtils.getItemImageUrl(mItem, mPodcastImage);
    ...
    private void initializeNotificationManager(Item item) {
        ...
        mPlayerNotificationManager.setSmallIcon(R.mipmap.ic_launcher);
        ...
        mPlayerNotificationManager.setFastForwardIncrementMs(Constants.FAST_FORWARD_INCREMENT);
        // Set the rewind increment by 10sec
        mPlayerNotificationManager.setRewindIncrementMs(Constants.REWIND_INCREMENT);
    ...
    private PendingIntent createContentPendingIntent(Item item) {
        ...
        Bundle b = new Bundle();
        b.putParcelable(Constants.EXTRA_ITEM, item); // Podcast episode
        intent.putExtra(Constants.EXTRA_ITEM, b);
        intent.putExtra(Constants.EXTRA_RESULT_NAME, mPodcastName); // Podcast title
        intent.putExtra(Constants.EXTRA_PODCAST_IMAGE, mPodcastImage); // Podcast Image

        return PendingIntent.getActivity(
                PodcastService.this, Constants.NOTIFICATION_PENDING_INTENT_ID,
    ...
    private class MySessionCallback extends MediaSessionCompat.Callback {
        ...
        public void onRewind() {
            mExoPlayer.seekTo(Math.max(mExoPlayer.getCurrentPosition() - Constants.REWIND_INCREMENT, 0));
        }
        ...
        public void onFastForward() {
            ...
            mExoPlayer.seekTo(Math.min(mExoPlayer.getCurrentPosition() +
                    Constants.FAST_FORWARD_INCREMENT, duration));
        }
    ...
    public class BitmapTask extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
            Bitmap bitmap = null;
            try {
                bitmap = PodCastUtils.loadImage(params[0]);
            } catch (IOException e) {
                e.printStackTrace();
            }
            return bitmap;
