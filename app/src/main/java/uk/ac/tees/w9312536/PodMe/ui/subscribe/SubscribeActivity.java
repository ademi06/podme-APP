...
import uk.ac.tees.w9312536.PodMe.model.LookupResult;
import uk.ac.tees.w9312536.PodMe.model.rss.ArtworkImage;
import uk.ac.tees.w9312536.PodMe.model.rss.Category;
import uk.ac.tees.w9312536.PodMe.model.rss.Channel;
...
import uk.ac.tees.w9312536.bukolafatunde.databinding.ActivitySubscribeBinding;
import uk.ac.tees.w9312536.PodMe.ui.detail.PodcastEntryViewModel;
...
public class SubscribeActivity extends AppCompatActivity {
    ...
    private int mVibrantColor = Constants.DEF_VIBRANT_COLOR;
    ...
    private void getResultData() {
        ...
        if (intent.hasExtra(Constants.EXTRA_RESULT_ID)) {
            mResultId = intent.getStringExtra(Constants.EXTRA_RESULT_ID);
        }
        if(intent.hasExtra(Constants.EXTRA_RESULT_NAME)) {
            mResultName = intent.getStringExtra(Constants.EXTRA_RESULT_NAME);
        }
        if(intent.hasExtra(Constants.EXTRA_RESULT_ARTWORK_100)) {
            mResultArtwork100 = intent.getStringExtra(Constants.EXTRA_RESULT_ARTWORK_100);
        }
    }
    ...
    private void setupViewModel() {
        SubscribeViewModelFactory factory = InjectorUtils.provideSubscribeViewModelFactory(
                SubscribeActivity.this, Constants.I_TUNES_LOOKUP, mResultId);
    ...
    private void showDetails(Channel channel) {
                ...
                .into(new ImageViewTarget<Bitmap>(mSubscribeBinding.ivPalette) {
                    ...
                    protected void setResource(@Nullable Bitmap resource) {
                        if (resource != null) {
                            ...
                            Palette.from(resource).maximumColorCount(Constants.MAX_COLOR_COUNT).generate(new Palette.PaletteAsyncListener() {
                                @Override
        ...
        if (!TextUtils.isEmpty(language)) {
            ...
            String languageName = PodCastUtils.convertLanguageCode(language);
